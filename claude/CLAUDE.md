...
> Última evolución: <!-- LAST_EVOLUTION -->2026-07-26 15:59<!-- /LAST_EVOLUTION -->
...
- [2026-07-26] El protocolo de cierre no corria con `exit`: el hook vivia en UserPromptSubmit y Claude Code intercepta `exit` como comando, nunca llega como prompt (las frases tipo 'eso es todo helix' si funcionaban). Fix: registrar el evento SessionEnd + marcador por session_id para no duplicar. Un script no debe terminar en un condicional: session-end.sh cerraba con [[ -f ... ]] y devolvia 1. Detalle: `topics/auditoria-cableado-20260726.md`
<!-- OPERABILITY_END -->
...
| 64 | 2026-07-26 | operatividad | El protocolo de cierre no corria con 'exit': session-exit-hook vive en UserPromptSubmit y Claude Code intercepta 'exit' como comando de salida, asi que nunca llega como prompt. Frases como 'eso es todo helix' si funcionaban por ser prompts normales. Fix: registrar el evento SessionEnd (soportado por el binario y sin usar en Helix, igual que PreCompact, SessionStart, SubagentStop y Notification) con un hook propio, mas un marcador por session_id compartido con el hook de prompt para que session-end.sh corra UNA vez si el usuario escribe la frase y ademas sale. Bonus encontrado: session-end.sh terminaba en un [[ -f helix-alerta.md ]] y sin ese archivo el test devolvia 1, que se volvia el exit code del script entero — todo caller lo leia como fallo. Leccion: un script no debe terminar en un condicional; cerrar con exit 0 explicito | exit-no-dispara-cierre |
<!-- EVOLUTION_LOG_END -->
